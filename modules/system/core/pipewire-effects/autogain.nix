# Advanced Auto Gain — multi-stage audio processing via PipeWire filter-chain.
#
# Source: Advanced Auto Gain.json (EasyEffects preset)
# Chain:  Parametric EQ → Calf Exciter (LV2) → EBU R128 Autogain → LSP Limiter (LV2)
#
# Creates a virtual sink named "Advanced Auto Gain" that appears as a
# selectable audio output device in desktop volume controls.
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.my.system.core.pipewire.pipewireEffects.autogain;

  eqFilters = [
    # input-gain  = -0.3 dB → preamp via highshelf at 0 Hz
    { type = "bq_highshelf"; freq = 0;        q = 1.0;  gain = -0.3; }
    # band7–band12: +4 dB boost (113–358 Hz)
    { type = "bq_peaking";   freq = 113.21;   q = 4.36; gain = 4.0; }
    { type = "bq_peaking";   freq = 142.53;   q = 4.36; gain = 4.0; }
    { type = "bq_peaking";   freq = 179.43;   q = 4.36; gain = 4.0; }
    { type = "bq_peaking";   freq = 225.89;   q = 4.36; gain = 4.0; }
    { type = "bq_peaking";   freq = 284.38;   q = 4.36; gain = 4.0; }
    { type = "bq_peaking";   freq = 358.02;   q = 4.36; gain = 4.0; }
    # band15–band19: various cuts (714–1794 Hz)
    { type = "bq_peaking";   freq = 714.34;   q = 4.36; gain = -1.0; }
    { type = "bq_peaking";   freq = 899.29;   q = 4.36; gain = -2.0; }
    { type = "bq_peaking";   freq = 1132.15;  q = 4.36; gain = -3.6; }
    { type = "bq_peaking";   freq = 1425.29;  q = 4.36; gain = -2.5; }
    { type = "bq_peaking";   freq = 1794.33;  q = 4.36; gain = -1.5; }
    # output-gain = -6.5 dB → postamp via highshelf at 0 Hz
    { type = "bq_highshelf"; freq = 0;        q = 1.0;  gain = -6.5; }
  ];
...
in
{
  options.my.system.core.pipewire.pipewireEffects.autogain = {
    enable = lib.mkEnableOption "Advanced Auto Gain virtual sink (EQ + Exciter + Autogain + Limiter)";
  };
