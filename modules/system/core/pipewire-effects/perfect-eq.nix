# Perfect EQ — 10-band parametric equalizer via PipeWire builtin filter-chain.
#
# Source: Perfect EQ.json (EasyEffects preset)
# Chain:  Parametric EQ only (pure builtin, no external plugins required)
#
# Creates a virtual sink named "Perfect EQ" that appears as a selectable
# audio output device in desktop volume controls.
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.my.system.core.pipewire.pipewireEffects.perfectEq;
...
in
{
  options.my.system.core.pipewire.pipewireEffects.perfectEq = {
    enable = lib.mkEnableOption "Perfect EQ virtual sink (10-band parametric equalizer)";
  };
