...
in
{
  options.my.user.librewolf = {
    enable = selfLib.mkBoolOpt false "LibreWolf configuration for user";
  };
  ...
  config = lib.mkIf cfg.enable {
    programs.librewolf = {
      ...
      # Kebijakan global untuk manajemen ekstensi dan perlindungan dasar
      policies = {
        ExtensionSettings = {
          "*" = {
      ...
      settings = {
        ...
        "browser.startup.page" = 3; # Resume session
        "browser.tabs.inTitlebar" = 0;

        # --- KEAMANAN EKSTENSI ---
        # Mengizinkan instalasi ekstensi yang tidak ditandatangani (unsigned)
        ...
        # --- PRIVASI TAMBAHAN ---
        # LibreWolf sudah strict, tapi kita pastikan beberapa hal spesifik tetap aktif
        "privacy.resistFingerprinting" = true;
        "dom.security.https_only_mode" = true;
        "browser.send_pings" = false;
        "network.dns.disablePrefetch" = true;

        # Disable password manager bawaan (Gunakan Bitwarden)
        "signon.rememberSignons" = false;
      };
    };

    # Konfigurasi userChrome tetap sama untuk estetika tab
