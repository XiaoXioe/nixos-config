...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.gallery-dl = {
      ...
      settings = {
        "extractor" = {
          # Dasar folder unduhan (Gunakan path absolut atau tilde)
          "base-directory" = "~/CloudStorage/Gdrive_Akbar68_Enc/Gallery/";

          # Database Archive: mencegah download ulang
          "archive" = "~/.local/share/gallery-dl/archive.sqlite3";

          # Struktur Folder: Situs/Nama_User (Pisahkan elemen list dengan spasi)
          "directory" = [
            "{category}"
            "{user[name]}"
          ];
