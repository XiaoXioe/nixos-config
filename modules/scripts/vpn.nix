...
selfLib.mkModule {
  ...
  hmConfig = hmOpts: {
    programs.fish.functions = {
      ...
      _vpn-connect = {
        description = "Internal: pilih config, buat wireproxy, verifikasi";
        body = ''
          set -l pre_ip $argv[1]
      ...
      # ── Internal: get real IP & ASN (bypassing proxy) ──────────────
      _vpn-get-identity = {
        description = "Internal: ambil IP dan ASN asli (bypass proxy)";
        body = ''
          set -g _vpn_pre_ip (env ALL_PROXY= ${pkgs.curl}/bin/curl -s --max-time 3 https://ifconfig.me; or echo "unknown")
          set -g _vpn_pre_asn (env ALL_PROXY= ${pkgs.curl}/bin/curl -s --max-time 3 https://ipinfo.io/org; or echo "unknown")
          if test "$_vpn_pre_asn" != "unknown"
              set -g _vpn_pre_asn (echo $_vpn_pre_asn | cut -d' ' -f 1)
          end
        '';
      };
      ...
      vpn-on = {
        ...
        body = ''
          ...
          if ${pkgs.procps}/bin/pgrep -u (id -u) -x wireproxy >/dev/null
              ...
              echo "✅ ALL_PROXY diarahkan ke 127.0.0.1:1080."
              if not _vpn-verify $_vpn_pre_ip $_vpn_pre_asn
                  return 1
              end
              return 0
          end

          _vpn-connect $_vpn_pre_ip $_vpn_pre_asn
        '';
      };
      ...
      vpn-switch = {
        ...
        body = ''
          ...
          if not _vpn-connect $_vpn_pre_ip $_vpn_pre_asn
              return 1
          end
