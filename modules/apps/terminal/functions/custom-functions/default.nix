    ...
    home.packages = [
      (selfLib.mkApp pkgs "cek_ip"
        ''
          ...
          if [ "$(echo "$response" | jq -r '.status' 2>/dev/null)" = "fail" ]; then
              echo "❌ Error: $(echo "$response" | jq -r '.message' 2>/dev/null)"
          else
              echo "$response" | jq -r 'to_entries | .[] | "\(.key) \(.value)"' | while read -r key value; do
                  case "$key" in
                      query) key="Target IP" ;;
                      status) continue ;;
                      country) key="Country" ;;
          ...
          if [ "$target_ip" != "$my_ip" ]; then
              ...
              nmap -F -T4 --open -Pn "$target_ip" 2>/dev/null | grep -E "^[0-9]+/tcp" | awk -v c_val="$c_val" '
                  { printf "  -> Port %-5s : %s%s\n", $1, c_val, $3 }
              '
