...
# ──────────
# detect_pkg_manager() is generated from distroDetectionRules table
@DETECT_PKG_MGR_BASH@
  ...
  case "$CONTAINER_PKG_MANAGER" in
    ...
    pacman)
      case "$GENERIC_ACTION" in
        ...
        autoremove) FINAL_CMD=(sh -c "pacman -Qdtq 2>/dev/null | xargs -r sudo pacman -Rns --noconfirm || echo '==> No orphan packages found.'"); USE_SUDO=false ;;
      esac
      ;;
    dnf)
      case "$GENERIC_ACTION" in
