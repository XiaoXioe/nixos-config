...
Canvas {
    ...
    visible: !Config.ready || !Config.options.performance.lowPerformanceMode
    layer.enabled: !Config.ready || !Config.options.performance.lowPerformanceMode

    onPointsChanged: {
        if (!Config.ready || !Config.options.performance.lowPerformanceMode) {
            root.requestPaint();
        }
    }
